...
const tokensLegend = new vscode.SemanticTokensLegend(
  rainbowTokenTypes,
  []
);
...
class MmCifTokenProvider implements vscode.DocumentSemanticTokensProvider {
   ...
   */
  private specialSplit(content: string): Array<[string, boolean]> {
   ...
   */
  private isDataName(token: [string, boolean]): boolean {
   ...
   */
  private isLoopKeyword(token: [string, boolean]): boolean {
   ...
   */
  private isBlockKeyword(token: [string, boolean]): boolean {
