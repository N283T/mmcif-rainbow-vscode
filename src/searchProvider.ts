        ...
        for (const loop of loops) {
            if (loop.categoryName && !categories.has(loop.categoryName)) {
                ...
                if (loop.fieldNames.length > 0) {
                    ...
                    // For highlighting, we want to highlight the full field name (e.g. _atom_site.id)
                    // or just the category part? User example: "_atom_site.group_PDB" (full name)
                    // But specifically said "highlight _atom_site.group_PDB part"

                    // Let's highlight the full field name range
